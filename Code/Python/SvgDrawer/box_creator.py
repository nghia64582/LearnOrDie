...
def export_svg():
    ...
    # Chọn thư mục gốc
    parent_folder = filedialog.askdirectory(title="Chọn thư mục lưu SVG")
    if not parent_folder:
        return

    # Tạo subfolder rec_<D>_<R>_<C>
    subfolder_name = f"rec_{int(D)}_{int(R)}_{int(C)}"
    ...
    messagebox.showinfo("Hoàn tất", f"Đã xuất 3 file SVG vào:\n{folder}")
