...
class Solution {
    ...
    private boolean dfs(char[][] board, boolean[][] visited, String word, int index, int i, int j) {
        ...
        visited[i][j] = true;
        if (dfs(board, visited, word, index + 1, i - 1, j) || dfs(board, visited, word, index + 1, i + 1, j) || dfs(board, visited, word, index + 1, i, j - 1) || dfs(board, visited, word, index + 1, i, j + 1)) {
            visited[i][j] = false;
            return true;
        }
        visited[i][j] = false;
