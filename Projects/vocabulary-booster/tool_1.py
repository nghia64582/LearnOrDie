...
def demo_wordfreq():
    ...
    words = ["school", "beautiful", "fascinating", "eye", "abatement", "hello", "supercalifragilisticexpialidocious", "three"]

    for word in words:
        zipf_score = zipf_frequency(word, 'en', wordlist='best')

        # Get the raw frequency (between 0 and 1, representing occurrence per billion words)
        raw_frequency = word_frequency(word, 'en', wordlist='best')

        print(f"'{word}':")
        print(f"  Zipf Score (0-~8 range): {zipf_score:.2f}")
        print(f"  Raw Frequency (per million words): {int(raw_frequency * 1_000_000)}") # Convert to per million for clarity
        print("-" * 30)
...
def analyze_word_frequencies(data):
    ...
    ids = set()
    for word in data:
        zipf_score = zipf_frequency(word["word"], 'en', wordlist='best')
        raw_frequency = word_frequency(word["word"], 'en', wordlist='best')
        word_id = _id(word["word"])
        if word_id in ids or '-' in word["word"] or ' ' in word["word"]:
            continue
        ids.add(word_id)
        result.append({
            "word": word["word"],
    ...
    result.sort(key=lambda x: x['zipf_score'], reverse=True)
    for word_data in result[10000:11000]:
        print(word_data)
...
if __name__ == "__main__":
    ...
    if parsed_data:
        ...
        analyze_word_frequencies(parsed_data)
    else:
